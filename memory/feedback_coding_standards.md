...
- All code MUST be written in English: class names, method names, variables, comments, Javadoc — everything. Conversation with the user stays in Spanish.
- Never use string/numeric literals inline in methods. Define them as `static final` constants at the top of the class, then reference those constants in the code. This applies to error messages, field names, default values, etc.
