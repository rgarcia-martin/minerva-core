...
- **Article**: supports a parent/child packaging model via `canHaveChildren`, `numberOfChildren`, `childArticleId`. The compact constructor enforces consistency (a parent article must declare both a positive child count and a non-self `childArticleId`; a non-parent must leave both null/zero).
...
- **Purchase → inventory expansion**: `PurchaseService.createInventoryItems(...)` generates one `Item` per unit in each `PurchaseLine`. When the purchased article `canHaveChildren` and the line uses `ItemStatus.OPENED`, `createChildItems(...)` splits the parent into `numberOfChildren` child items at `parentItem.cost / numberOfChildren` (HALF_UP, 2dp), each linked back via `parentItemId`. This is the "box-of-pens already opened" flow.
